...
if (action === 'getGrossProfitSummary') {
    ...
    var gpFrom = null;
    if (gpPeriod === 'day' || gpPeriod === 'week' || gpPeriod === 'month') {
      var gpNow = new Date();
      if (gpPeriod === 'month') {
    ...
    var result = { total: 0, clients: {}, period: gpPeriod, undated: 0, undatedJobs: 0,
...
if (action === 'scanPOEmails') {
  ...
  if (action === 'aiAsk') {
    ...
    var akRules =
      ...
      '(9) TWO data sources follow and they DO NOT AGREE BY CONSTRUCTION. The ALL JOBS rows are the complete raw data — every job, no filter. The BUSINESS CONTEXT block calls itself authoritative, but it (a) counts ONLY jobs with logged actual hours above a minimum, so it silently drops jobs, and (b) reports trueMargin = gross profit MINUS job expenses, which is NOT gross profit. ' +
      'Therefore: for any question about gross profit, revenue, costs, totals, or one client\'s figures — COMPUTE FROM THE ALL JOBS ROWS, not from BUSINESS CONTEXT. Use BUSINESS CONTEXT only for per-hour and target-gap comparisons, and whenever you quote a figure from it, say in one short clause that it counts only jobs with logged hours and is margin after expenses. Never label a trueMargin figure as רווח גולמי.';
...
function ybFullDump(forceRefresh) {
  var cache = CacheService.getScriptCache(), KEY = 'YB_FULL_DUMP_V227';
  ...
  try {
    if (typeof _geReadRows === 'function') {
      ...
      lines.push('=== TOTAL GENERAL EXPENSE ROWS: ' + ge.length + ' ===');
    }
