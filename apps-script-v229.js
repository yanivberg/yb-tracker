...
if (action === 'scanPOEmails') {
  ...
  if (action === 'aiAsk') {
    ...
    var akRules =
      ...
      'If a total you need is not in EXACT TOTALS, say אין לי את הנתון הזה rather than computing it. Simple arithmetic BETWEEN two figures already in EXACT TOTALS (a difference, a ratio) is fine. ' +
...
function ybFullDump(forceRefresh) {
  var cache = CacheService.getScriptCache(), KEY = 'YB_FULL_DUMP_V229';
  ...
  var TOT = {}, CAT = {}, MON = {}, PIPE = {}, GRAND = {jobs:0, income:0, costs:0, gross:0, hours:0, expenses:0};
  ...
  for (var s2 = 0; s2 < sheets.length; s2++) {
    ...
    for (var r = cols.headerRow + 1; r < data.length; r++) {
      ...
      if (stv229 === 'בוצע') {
        ...
        var dv229 = row[cols.dateEnd] || row[cols.dateStart];
        if (dv229 && dv229.getFullYear) {
          add229(bucket(MON, dv229.getFullYear() + '-' + ('0' + (dv229.getMonth() + 1)).slice(-2)));
        }
  ...
  lines.push('-- per month (by end date, falling back to start date) --');
