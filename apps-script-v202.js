 ...
 if (action === 'getExpenses') {
  ...
  if (action === 'deleteExpense') {
    try {
      ...
      if (jobId3) _updateJobCosts(ss, jobId3, client3);
      return ContentService.createTextOutput(JSON.stringify({ok:true})).setMimeType(ContentService.MimeType.JSON);
...
if (action === 'getYoman') {
  ...
  if (action === 'addPONewRow') {
    try {
      ...
      var sheet    = ss.getSheetByName(client);
      if (!sheet) return ContentService.createTextOutput('Sheet not found').setMimeType(ContentService.MimeType.TEXT);
      var jobId = addPORowAndReturn(sheet, client, desc, price, poNumber);
  ...
  if (action === 'createExpenseRow') {
    try {
      ...
      if (expSh) {
        var expData = expSh.getDataRange().getValues();
        for (var er = 1; er < expData.length; er++) {
          if (String(expData[er][0]).trim() === jobId) totalExp += parseFloat(expData[er][5]||0);
        }
      }
      ...
      // Build row
      var mc = 0;
      [cols.jobId,cols.desc,cols.price,cols.qty,cols.status,cols.po,cols.invoiceStatus,cols.dateStart,cols.income,cols.grossProfit].forEach(function(c){if(c>mc)mc=c;});
      ...
      return ContentService.createTextOutput(JSON.stringify({ok:true, newJobId:newJobId, total:totalExp})).setMimeType(ContentService.MimeType.JSON);
    } catch(ee) { return ContentService.createTextOutput(JSON.stringify({ok:false,error:ee.message})).setMimeType(ContentService.MimeType.JSON); }
  }
  return ContentService.createTextOutput('OK').setMimeType(ContentService.MimeType.TEXT);
}
...
function doPost(e) {
  ...
  if (action === 'updateQuote') {
    try {
      ...
      // ── Email via Caspit EmailDocument API ──────────────────────────────
      var docId4  = caspitDoc.DocumentId || '';
      var docNum4 = caspitDoc.Number     || '';
      var mailSent4 = false;
      try {
        var caspitToken4 = getCaspitToken();
        var emailUrl4 = 'https://app.caspit.biz/api/v1/EmailDocument/' +
          encodeURIComponent(docId4) +
          '?token=' + encodeURIComponent(caspitToken4) +
          '&documentNumber=' + encodeURIComponent(docNum4) +
          '&trxTypeId=16&lan=0';
        var emailResp4 = UrlFetchApp.fetch(emailUrl4, {
          method: 'post',
          contentType: 'application/json',
          payload: JSON.stringify({ To: 'yanivberg@icloud.com' }),
          muteHttpExceptions: true, deadline: 20
        });
        mailSent4 = emailResp4.getResponseCode() < 300;
      } catch(_emailErr) {}
  ...
  if (action === 'emailQuote') {
    // v201: email a quote's PDF via Caspit EmailDocument — called by HTML v921 AFTER it has
    // cleaned the auto-filled line Details, so the mailed PDF never shows the double description.
    var docIdEQ  = (e.parameter && e.parameter.docId)  || body.docId  || '';
    var docNumEQ = (e.parameter && e.parameter.docNum) || body.docNum || '';
    if (!docIdEQ) return ContentService.createTextOutput(JSON.stringify({ok:false,error:'docId required'})).setMimeType(ContentService.MimeType.JSON);
    try {
      var tokenEQ = getCaspitToken();
      var urlEQ = 'https://app.caspit.biz/api/v1/EmailDocument/' + encodeURIComponent(docIdEQ) +
        '?token=' + encodeURIComponent(tokenEQ) +
        '&documentNumber=' + encodeURIComponent(docNumEQ) + '&trxTypeId=16&lan=0';
      var respEQ = UrlFetchApp.fetch(urlEQ, {
        method: 'post', contentType: 'application/json',
        payload: JSON.stringify({ To: 'yanivberg@icloud.com' }),
        muteHttpExceptions: true, deadline: 20
      });
      // v202: stale-token retry — clear cache, fresh token, one retry
      if (respEQ.getResponseCode() >= 400) {
        PropertiesService.getScriptProperties().deleteProperty('CASPIT_TOKEN');
        var tokenEQ2 = getCaspitToken();
        var urlEQ2 = 'https://app.caspit.biz/api/v1/EmailDocument/' + encodeURIComponent(docIdEQ) +
          '?token=' + encodeURIComponent(tokenEQ2) +
          '&documentNumber=' + encodeURIComponent(docNumEQ) + '&trxTypeId=16&lan=0';
        respEQ = UrlFetchApp.fetch(urlEQ2, {
          method: 'post', contentType: 'application/json',
          payload: JSON.stringify({ To: 'yanivberg@icloud.com' }),
          muteHttpExceptions: true, deadline: 20
        });
      }
      return ContentService.createTextOutput(JSON.stringify({ok: respEQ.getResponseCode() < 300, code: respEQ.getResponseCode()})).setMimeType(ContentService.MimeType.JSON);
...
function addPORowAndReturn(sheet, clientName, description, price, poNumber) {
  var data = sheet.getDataRange().getValues();
  var cols = findColumns(data);
  ...
  if (targetRow !== -1) {
    ...
    if (cols.qty    !== -1) sheet.getRange(wRow, cols.qty+1).setValue(1);
