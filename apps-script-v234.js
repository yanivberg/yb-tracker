// Apps Script — v234
// v234: getGrossProfitSummary undated=include|separate. 118 completed jobs / ~319k have
//       NO date in the sheet at all (not a parsing issue — the cells are empty), so any
//       period total understates reality. 'include' folds them into the total while still
//       reporting result.undated, so the UI can show how much of the figure is undated.
//       ⚠ Meaningful for year/all; for day/week it inflates a short window with jobs that
//       may belong to any date — the UI must say so.
...
if (action === 'getGrossProfitSummary') {
    ...
    /* v234: undated=include folds date-less jobs INTO the total (still reported
       separately so the UI can say how much of it is undated). Default 'separate'
       keeps the old behaviour. Only meaningful with a period window. */
    var gpUndated = String(e.parameter.undated || 'separate').toLowerCase();
    if (gpUndated !== 'include') gpUndated = 'separate';
    var gpScope = String(e.parameter.scope || 'toinvoice').toLowerCase();
    if (gpScope !== 'done') gpScope = 'toinvoice';
    var result = { total: 0, clients: {}, period: gpPeriod, scope: gpScope, undatedMode: gpUndated, undated: 0, undatedJobs: 0,
    ...
    ss.getSheets().forEach(function(sheet) {
      ...
      for (var r = cols.headerRow + 1; r < data.length; r++) {
        ...
        if (gpFrom) {
          ...
          if (!gpDate) {
            result.undated += gp; result.undatedJobs++;
            if (gpUndated !== 'include') continue;   /* v234: 'include' lets it fall through into the total */
          } else if (gpDate < gpFrom) continue;
        }
        result.total += gp;
