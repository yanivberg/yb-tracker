...
function _buildMonthlyListPdf(allDocs, month) {
  ...
  if (action === 'createExpenseRow') {
    try {
      ...
      if (_syncExpenseRollup(ss, jobId, client, title)) return ContentService.createTextOutput(JSON.stringify({ok:true, updated:true, total:totalExp})).setMimeType(ContentService.MimeType.JSON);
